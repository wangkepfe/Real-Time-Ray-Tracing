...
Float3 Halfedge_Mesh::Vertex::neighborhood_center() const {
...
Float3 Halfedge_Mesh::Vertex::normal() const {
    Float3 n;
    Float3 pi = pos;
    HalfedgeCRef h = halfedge();
    if(on_boundary()) {
        do {
            Float3 pj = h->next()->vertex()->pos;
            Float3 pk = h->next()->next()->vertex()->pos;
            n += cross(pj - pi, pk - pi);
            h = h->next()->twin();
        } while(h != halfedge());
    } else {
...
Float3 Halfedge_Mesh::Edge::normal() const {
    return (halfedge()->face()->normal() + halfedge()->twin()->face()->normal()).normalized();
}

Float3 Halfedge_Mesh::Face::normal() const {
    Float3 n;
    HalfedgeCRef h = halfedge();
    do {
        Float3 pi = h->vertex()->pos;
        Float3 pj = h->next()->vertex()->pos;
        n += cross(pi, pj);
        h = h->next();
    } while(h != halfedge());
...
Float3 Halfedge_Mesh::Vertex::center() const {
    return pos;
}
...
Float3 Halfedge_Mesh::Edge::center() const {
    return 0.5f * (_halfedge->vertex()->pos + _halfedge->twin()->vertex()->pos);
}

Float3 Halfedge_Mesh::Face::center() const {
    Float3 c;
    float d = 0.0f;
    HalfedgeCRef h = _halfedge;
    do {
...
Float3 Halfedge_Mesh::normal_of(Halfedge_Mesh::ElementRef elem) {
    Float3 n;
...
Float3 Halfedge_Mesh::center_of(Halfedge_Mesh::ElementRef elem) {
    Float3 pos;
...
void Halfedge_Mesh::to_mesh(GL::Mesh& mesh, bool split_faces) const {
    ...
    if(split_faces) {
        ...
        for(FaceCRef f = faces_begin(); f != faces_end(); f++) {
            ...
            Float3 v0 = face_verts[0].pos;
            for(size_t i = 1; i <= face_verts.size() - 2; i++) {
                Float3 v1 = face_verts[i].pos;
                Float3 v2 = face_verts[i + 1].pos;
                Float3 n = cross(v1 - v0, v2 - v0).normalized();
                if(flip_orientation) n = -n;
...
std::string Halfedge_Mesh::from_mesh(const GL::Mesh& mesh) {
    ...
    std::vector<Float3> verts(v.size());
...
void Halfedge_Mesh::to_triangles(std::vector<Triangle>& triangles)
{
    for (const auto& f : faces)
    {
        auto h = f.halfedge();

        auto v0 = h->vertex()->pos; h = h->next();
        auto v1 = h->vertex()->pos; h = h->next();
        auto v2 = h->vertex()->pos; h = h->next();
        auto v3 = h->vertex()->pos;

        if (v0 == v3) // triangle
        {
            std::cout << "triangle found in hafe edge mesh!\n";
        }
        else
        {
            triangles.push_back(Triangle(v0, v1, v2));
            triangles.push_back(Triangle(v2, v3, v0));
        }
    }
}