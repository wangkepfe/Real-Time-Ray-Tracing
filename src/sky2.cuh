...
struct SkyParams2
{
	...
	// Mie
	float mfpKmMie = 43.478f;
	float albedoMie = 0.91f;
	float g = 0.76;
...
inline __device__ Float3 GetEnvIncidentLight(const Float3& raydir, const Float3& sunDir, SkyParams& skyParams)
{
	...
	// bool printCondition = (raydir.z > 0.0f && raydir.x < 0.01f && raydir.z > 0.99f && raydir.y > 0.0f && raydir.y < 0.1f);
	// bool printCondition = (raydir.x > 0.0f && raydir.x < 0.01f && raydir.z > 0.99f && raydir.y > 0.0f && raydir.y < 0.1f);

	// if (printCondition)
	// {
	// 	Print("raydir", raydir);
	// 	Print("marchStep", marchStep);
	// 	Print("t", t);
	// }
	...
	Float3 result = sunPower * (sumR * phaseRayleigh + sumM * phaseMie);

	// if (printCondition)
	// {
	// 	printf("result = (%f, %f, %f), raydir = (%f, %f, %f)\n", result[0], result[1], result[2], raydir[0], raydir[1], raydir[2]);
	// }
...
__global__ void Sky(SurfObj skyBuffer, float* skyPdf, Int2 size, Float3 sunDir, SkyParams skyParams)
{
	...
	Float3 color = GetEnvIncidentLight(rayDir, sunDir, skyParams);
