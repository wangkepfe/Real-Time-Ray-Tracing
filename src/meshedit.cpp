...
*/
void Halfedge_Mesh::bevel_vertex_positions(const std::vector<Float3>& start_positions,
...
*/
void Halfedge_Mesh::bevel_edge_positions(const std::vector<Float3>& start_positions,
                                         Halfedge_Mesh::FaceRef face, float tangent_offset) {
...
/*
    ...
    for(size_t i = 0; i < new_halfedges.size(); i++)
    {
            Float3 pi = start_positions[i]; // get the original vertex
            position corresponding to vertex i
    }
*/
void Halfedge_Mesh::bevel_face_positions(const std::vector<Float3>& start_positions,
                                         Halfedge_Mesh::FaceRef face, float tangent_offset,
                                         float normal_offset) {
...
void Halfedge_Mesh::catmullclark_subdivide_positions() {
    ...
    // Edges
    for (auto& e : edges) {
        e.new_pos = e.center() / 2 + 
                    e.halfedge()->face()->new_pos / 4 + 
    ...
    for (auto& v : vertices) {
        ...
        Float3 sum_faces_center;
        Float3 sum_edges_center;
        HalfedgeCRef h = v.halfedge();

        do {
            h = h->twin()->next();
            sum_faces_center += h->face()->new_pos / n;
            sum_edges_center += h->edge()->new_pos / n;
        } while (h != v.halfedge());

        v.new_pos = (sum_faces_center + 2 * sum_edges_center + v.pos * (n - 3)) / n;
    }
}
