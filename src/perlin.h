...
public:
    ...
    float noise(float x, float y) const
    {
        ...
        // Add blended results from 8 corners of cube
        float res = lerp(w, lerp(v, lerp(u, grad(p[AA], x, y, 0), grad(p[BA], x-1, y, 0)), lerp(u, grad(p[AB], x, y-1, 0), grad(p[BB], x-1, y-1, 0))),	lerp(v, lerp(u, grad(p[AA+1], x, y, 0-1), grad(p[BA+1], x-1, y, 0-1)), lerp(u, grad(p[AB+1], x, y-1, 0-1),	grad(p[BB+1], x-1, y-1, 0-1))));
        return (res + 1.0f)/2.0f;
    }
private:
    std::vector<int> p;
